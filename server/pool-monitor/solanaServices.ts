 ...
 * Creates Switchboard Randomness Account, commits, and calls request_randomness instruction
 */
export async function requestRandomnessOnChain(poolAddress: string): Promise<void> {
  ...
  try {
    ...
    // Step 3: Create pool's request_randomness instruction
    const [participantsPda] = deriveParticipantsPda(poolPk);
    ...
    const requestIx = createInstructionWithDiscriminator(
      ...
      Buffer.alloc(0),
      [
        { pubkey: rngObj.pubkey, isSigner: false, isWritable: false },
        { pubkey: poolPk, isSigner: false, isWritable: true },
    ...
    // Transaction 3: REQUEST randomness on pool
    const tx3 = new Transaction();
    const bh3 = await conn.getLatestBlockhash();
    ...
    log(`pool=${poolAddress.slice(0, 8)} action=REQUEST_RANDOMNESS ✅ TX_SENT=${sig3.slice(0, 16)}`);
    await conn.confirmTransaction(sig3, "confirmed");
    log(`pool=${poolAddress.slice(0, 8)} action=REQUEST_RANDOMNESS ✅ TX_CONFIRMED=${sig3.slice(0, 16)} rng=${rngObj.pubkey.toBase58().slice(0, 16)}`);
  } catch (err: any) {
 ...
 * Calls revealIx() on the Switchboard randomness account
 */
 ...
 * Uses select_winner instruction
 */
